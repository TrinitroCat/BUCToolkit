    ...
    def run(self,
        ...
        for i in range(self.maxiter_trans):
            ...
            for j in range(self.maxiter_rot):
                ...
                if is_grad_func_contain_y:
                    F2 = - grad_func_(X2, y2, *grad_func_args, **grad_func_kwargs) * atom_masks
                else:
                    F2 = - grad_func_(X2, *grad_func_args, **grad_func_kwargs) * atom_masks
                del y1, y2
                F1_ = F1.flatten(-2, -1)
                F2_ = F2.flatten(-2, -1)
                F_tol_ = (F1_ + F2_)/2.
                ...
                if th.all(converge_mask):  # judge transition state
                    ...
                    break
                if th.all(Torque < self.F_threshold):  # judge rot
                ...
                if is_grad_func_contain_y:
                    F1_d = - grad_func_(X1_d, y1_d, *grad_func_args, **grad_func_kwargs) * atom_masks
                    F2_d = - grad_func_(X2_d, y2_d, *grad_func_args, **grad_func_kwargs) * atom_masks
                else:
                ...
                del y1_d, y2_d
                F1_d_ = F1_d.flatten(-2, -1)
                F2_d_ = F2_d.flatten(-2, -1)
                Fc_d_ = F1_d_ - F2_d_
            ...
            F2_d_ = F2_d.flatten(-2, -1)
            Fc_d_ = (F1_d_ + F2_d_)/2.
            Fc_d_hori_ = th.where(Curv > 0.,
                               - (Fc_d_.unsqueeze(1) @ NI_.unsqueeze(-1)).squeeze(-1) * NI_,
                               (Fc_d_ - 2 * (Fc_d_.unsqueeze(1) @ NI_.unsqueeze(-1)).squeeze(-1)) * NI_)  # (n_batch, n_atom * n_dim)
