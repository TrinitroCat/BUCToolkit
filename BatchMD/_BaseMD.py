...
from .._Masses import MASS, N_MASS, ATOMIC_NUMBER
...
class _BaseMD:
    ...
    def run(self,
            func: Any | nn.Module, X: th.Tensor, Element_list: List[List[str]] | List[List[int]], V_init: th.Tensor | None = None, grad_func: Any | nn.Module = None,
        ...
        Parameters:
            ...
            V_init: Tensor | None, the initial velocities of each atom. if None, a random velocity generated by Boltzmann distribution would be set.
        ...
        self.EK_TARGET = (n_dim / 2.) * (n_atom - 1) * 8.617333262145e-5 * self.T_init  # Unit: eV/atom. Boltzmann constant kB = 8.617333262145e-5 eV/K
        # other check
        ...
        for i in range(self.max_step):
            ...
            if self.verbose > 0:
                np.set_printoptions(precision=8, linewidth=120, floatmode='fixed', suppress=True, formatter={'float': '{:> .8e}'.format}, threshold=2000)
                ...
                for _i, __x in enumerate(V_tup):
                    Ek[_i] = 0.5 * th.sum(masses_tup[_i] * __x ** 2, dim=(-2, -1)) * 103.642696562621738  # (n_batch, ), eV/atom. Faraday constant F = 96485.3321233100184.
                    temperature[_i] = (2 * Ek[_i]) / (n_dim * (__x.shape[1] - 1 + 1e-20) * 8.617333262145e-5)  # Boltzmann constant kB = 8.617333262145e-5 eV/K
        ...
        if self.verbose > 0:
            ...
            if batch_indices is not None:
                ...
                V_tup = (V,)
            if self.verbose > 1:
                np.set_printoptions(precision=8, linewidth=120, floatmode='fixed', suppress=True, formatter={'float': '{:> 5.10f}'.format}, threshold=3000000)
                self.logger.info('_' * 100)
