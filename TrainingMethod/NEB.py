...
from BM4Ckit.BatchOptim.TS.CI_NEB import CI_NEB
...
from BM4Ckit.utils.AtomSelector import atom_fix_selector
    ...
    def run(self, model):
        ...
        try:
            ...
            if self.VERBOSE > 0:
                ...
                self.logger.info(' TASK: Nudged Elastic Band Transition State Searching <<')
                ...
                self.logger.info(f'\tALGORITHM: {self.NEB.get('ALGO', 'CI-NEB')}')
                for _algo_conf_name, _algo_conf in self.NEB_config.items():
